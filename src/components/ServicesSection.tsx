...
export default function ServicesSection() {
  ...
  return (
    <section id="services" ref={ref} className="py-32" style={{ background: '#0B1426' }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="text-center mb-16">
          <div className="font-inter text-[11px] tracking-[0.18em] text-gold mb-4">OUR SERVICES</div>
          <h2 className="font-playfair text-ivory mb-6" style={{ fontSize: 'clamp(32px, 4vw, 52px)' }}>
            Three proven strategies. One trusted partner.
          </h2>
          <p className="font-inter text-[18px] max-w-2xl mx-auto" style={{ color: 'rgba(248,246,242,0.65)' }}>
            Every Abodoo client is matched to the strategy that maximises their property's income potential.
          </p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-7">
          {cards.map((c, i) => (
            <motion.div key={i}
              ...
              <div className="inline-block rounded-full px-3 py-1 font-inter text-[11px] tracking-widest text-gold mb-5"
                style={{ background: 'rgba(198,169,107,0.09)', border: '1px solid rgba(198,169,107,0.25)' }}>
                {c.pill}
              </div>
              <c.icon />
              <h3 className="font-playfair text-[24px] text-ivory mt-4">{c.title}</h3>
              <p className="font-inter text-[16px] leading-relaxed mt-3" style={{ color: 'rgba(248,246,242,0.65)' }}>{c.body}</p>
              <ul className="mt-4 space-y-2">
                {c.list.map((l, j) => (
                  <li key={j} className="font-inter text-[14px] flex" style={{ color: 'rgba(248,246,242,0.6)' }}>
                    <span className="text-gold mr-2">—</span>{l}
                  </li>
                ))}
              </ul>
              <div className="inline-block rounded-full px-4 py-1.5 font-inter text-[12px] text-gold mt-5"
                style={{ background: 'rgba(198,169,107,0.07)', border: '1px solid rgba(198,169,107,0.2)' }}>
                {c.badge}
              </div>
              <a href="#contact" className="block mt-4 font-inter text-[13px] text-gold hover:underline cursor-pointer">{c.link}</a>
            </motion.div>
          ))}
        </div>
      </div>
